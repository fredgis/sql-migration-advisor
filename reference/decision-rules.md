...
Source of truth: `docs/sql-server-to-azure-migration.md` (sql-migration-advisor), **v1.9**, verified August 2026.
...
| `source_os` | Windows Server ≤ 2012 R2 · Windows Server 2016+ · Linux · unknown. Gates MI Link, which requires **Windows Server 2016 or later**. |
...
| **Azure SQL Managed Instance (MI)** | FILESTREAM/FileTable; PolyBase to external RDBMS; heterogeneous DTC to third-party RDBMS; need OS/file-system access; unsupported third-party linked server dependency; **Service Broker requiring cross-instance routing** | SQL Agent jobs usually native; **Service Broker intra-instance is eligible**; SQL CLR/cross-DB usually compatible but assess; cloud-file PolyBase eligible; homogeneous SQL↔SQL DTC eligible | PaaS lift-and-shift for instance features. Service Broker is supported **within a single instance only**; cross-instance routing must be consolidated or refactored, and unknown scope requires a topology assessment. |
...
| Downtime wanted | Method | Gate |
| --- | --- | --- |
| Near-zero / online | **MI Link** | SQL Server 2016+, **Windows Server 2016+**, **Enterprise / Standard / Developer edition**, sysadmin, distributed AG, AG endpoint creation, required 5022 + 11000–11999 ports, VNet connectivity; not possible from AWS RDS/GCP Cloud SQL. Unknown OS or edition makes the method `unknown_requires_assessment`; an unsupported OS or edition eliminates **MI Link only**, never the MI target |

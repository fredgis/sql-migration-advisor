...
- `confidence` is **unaffected** when a candidate is `unknown_requires_assessment` **because no question in this interview reaches the blocking prerequisites of its path**. No answer the user could give would change it, so lowering confidence would punish the profile for where the two skills divide. The candidate is still held, and the work is named in `evidenceRequired` and `nextActions` for the prerequisite-plan skill. Output-contract invariant 5b defines both kinds; this line is the same rule stated where the confidence is derived. Without the distinction, a profile that answered every question would still read `low`, which makes the `medium` ceiling above unreachable and stops the scale from distinguishing anything.

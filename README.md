...
- **Rules under regression test** — Phase A filters hard eligibility, then Phase B ranks viable options and tiers. An executable mirror in `tests/` replays 117 scenarios through those rules on every commit. The mirror is not what runs in your session: an agent reads the rules and applies them, so this is a tested policy rather than a byte-identical guarantee.
...
- **Regression protection** — [`tests/`](tests/) holds 117 golden scenarios and 75 gates wired into CI, plus a branch-coverage floor on the decision engine so a gate cannot exist over code no scenario reaches.
...
| `SKILL.md` and its pinned fetch URL | `v3.14.5` | ✅ |
| `version.json`, `plugin.json`, `marketplace.json` | `v3.14.5` | ✅ |
...
| [Microsoft fork](https://github.com/microsoft/sql-migration-agent) | `v3.14.5` | ✅ |
| [Published rule graph](https://fredgis.github.io/sql-migration-advisor/rule-graph.html) | `v3.14.5` | ✅ |
| `howto/*.html` | — | ✅ |
...
| [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) | Deep dive: what the plugin is, how a session runs end to end, what the 75 gates defend, and where it can still be wrong. |
...
| **Tested coverage** | What the 117 golden scenarios actually reach |

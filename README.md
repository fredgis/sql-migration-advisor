...
<p align="center">
  ...
  <img alt="Knowledge base v3.14" src="https://img.shields.io/badge/knowledge%20base-v3.14-2b8a3e">
...
- **Verified knowledge** — the v3.14 knowledge base is source-backed and corrected against Microsoft Learn.
- **Rules under regression test** — Phase A filters hard eligibility, then Phase B ranks viable options and tiers. An executable mirror in `tests/` replays 118 scenarios through those rules on every commit. The mirror is not what runs in your session: an agent reads the rules and applies them, so this is a tested policy rather than a byte-identical guarantee.
...
- **Regression protection** — [`tests/`](tests/) holds 118 golden scenarios and 76 gates wired into CI, plus a branch-coverage floor on the decision engine so a gate cannot exist over code no scenario reaches.
...
| --- | --- | --- |
| [Knowledge base](docs/sql-server-to-azure-migration.md) | `v3.14` | ✅ |
| [`reference/decision-rules.md`](reference/decision-rules.md) + `.data.json` | `v3.14` | ✅ |
| `SKILL.md` and its pinned fetch URL | `v3.14.8` | ✅ |
| `version.json`, `plugin.json`, `marketplace.json` | `v3.14.8` | ✅ |
| [PDF](docs/sql-server-to-azure-migration.pdf) and its preview image | `v3.14` | ✅ |
| Poster caption and PNG | `v3.14` | ✅ |
| This README's badge and PDF sentence | `v3.14` | ✅ |
| **This table** | `v3.14` | ✅ |
| The six `blume/public/*.svg` mirrors | — | ✅ |
| [`blume/docs/index.mdx`](blume/docs/index.mdx) — the docs site | — | ✅ |
| [Microsoft fork](https://github.com/microsoft/sql-migration-agent) | `v3.14.8` | ✅ |
| [Published rule graph](https://fredgis.github.io/sql-migration-advisor/rule-graph.html) | `v3.14.8` | ✅ |
| `howto/*.html` | — | ✅ |
| The developer pitch's sample failure block | `v3.14` | ✅ |
...
| [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) | Deep dive: what the plugin is, how a session runs end to end, what the 76 gates defend, and where it can still be wrong. |
...
12 common requirements and 291 rows in total, each with a stable ID, an owner, an applicability
...
| **Tested coverage** | What the 118 golden scenarios actually reach |
...
v3.14, September 2026) — ready to hand to a partner or attach to a deal. It's generated reproducibly
...
| --- | --- | --- |
| v3.14.8 | 2026-09-13 | **One status, two reasons, and only one of them was written down.** A candidate held at `unknown_requires_assessment` can be waiting on a field the profile does not carry, or on a prerequisite path no question in this interview reaches. The first is a hard-gate unknown and belongs in both `unknowns` and `evidenceRequired`. The second is not, and the previous release put it in `evidenceRequired` alone without saying why, which is a third category the contract never defined. Recording it as an unknown was the alternative, and it was measured: 29 scenarios drop to `low`, including both `evidence-complete` scenarios, which state every field their name claims. A profile that answers everything would still read `low`, so the documented `medium` ceiling would become unreachable and the confidence scale would stop distinguishing anything. Invariant 5b names the two reasons instead: the field kind lowers confidence because the interview could have settled it, the path kind does not because no answer the user could give would change it, and it goes to `evidenceRequired` and `nextActions` as work for the prerequisite-plan skill. Writing that found the real hole: several rules hold a candidate and record their reason under the winning method name or under none, so a held candidate could carry its reason in its own text and nowhere a reader looks. A gate reads all 269 held candidates across the scenario set and fails on any whose reason appears in none of the three arrays. 74 gates to 75. |
| v3.14.4 | 2026-09-13 | **Four findings on the third state introduced one release earlier.** A gate that could not check a fact returned the same string as a gate that refused one, so every caller read both as a refusal: DMS came back `unavailable` on a profile where nobody had collected the recovery model, a word that asserts the source was examined and failed, and it took the offline `P23` variant down with it and could force a switch to SQL VM. Refusals and unchecked gates are distinguished now, and an unchecked gate leaves the candidate and the fallback standing. Second, the unproven-prerequisite check keyed on the fields the crosswalk bridges, so a path whose questions are all unbridged had nothing to test: transactional replication was offered as `available` with none of `P13` established. A path the interview cannot reach at all is named as such. Third, those checks pushed an evidence line without the matching `unknowns` entry, so the always-loaded exemplar failed invariant 5, its own pre-render self-check. Both arrays carry it, and the two `evidence-complete` scenarios now state the method prerequisites their name claims. Fourth, two schema descriptions still said invariant 15 requires the selected candidate to be `available`, while the output contract permits a winner at `unknown_requires_assessment` with a matching gate, which is what both canonical examples now are: a conforming run read opposite definitions of the same rule. |
| v3.14.3 | 2026-09-12 | **Four review findings, and the first was a bridge built four hours earlier that cleared eight blockers on one sentence.** `log_chain_status: CHAIN_INTACT` was mapped to `backup_chain_status: CONFIRMED`. That field feeds eight blocking prerequisites whose evidence columns ask for `RESTORE VERIFYONLY`, a gap check and a restore rehearsal, and the consumer is told never to re-ask an inherited fact, so a user saying the chain was unbroken settled all eight and moved a never-tested restore route off `blocked`. The mapping is asymmetric now, because the two directions are not equally conclusive: a reported break is proof of a problem and stays `MISSING`, an intact chain is an input to proof and converts to nothing. Second, the always-loaded exemplar left `fabric_sql_db` eligible for a profile with linked servers while the same trace refused Azure SQL Database for them, on a reason that argues ranking. SQL database in Fabric is database-scoped and narrower, so it now inherits any verdict SQL Database receives and cannot be the more permissive of the two; ten scenarios moved with it. Third, the error table still said the bundled copy is "always in context", which contradicts step 1 and lets a model skip the mandatory reads and rebuild the rules from memory, defeating the stop-on-read-failure rule. The row now points back at step 1, and step 1 names all seven files rather than three, including both schemas and `advisor-coverage.json`, which the candidate list cannot be built without. Fourth, that exemplar marked five methods `available` while its profile stated none of the facts their paths turn on. A candidate whose blocking prerequisites are unstated reads `unknown_requires_assessment` now, derived from the catalog, the questions and the crosswalk rather than written per method, and the missing fields reach `evidenceRequired` by name. |
| v3.14.2 | 2026-09-12 | **A bad edit shipped the correction and the mistake, four lines apart, in the always-loaded file.** Rewriting the method-candidate template in v3.14.1 left the previous draft behind instead of replacing it, so `SKILL.md` carried the same two paragraphs twice and the stale copy told the agent to write `unverified`, a status neither the output schema nor the output contract defines. The duplicate is gone and a gate now reads the two status enums out of the schema and refuses any other word in that block, and refuses a paragraph that appears twice in it. 73 gates to 74. |
| v3.14.1 | 2026-09-12 | **The document the model reads said the opposite of the schema that judges it.** v3.14.0 replaced the evidence model everywhere it could be executed: the schema, the validator, the exemplar plan. It left `output-contract.md`, which is the one of the four a session actually opens. Invariant 22 still sent `acceptedEvidence` ids to `sourceRegister`, a list of citations that cannot hold an evidence record, so a plan that followed the contract produced output the schema then rejected. The same round of review found the method list offering two states, `available` and `unavailable`, four lines below a gate that argues at length for three: a method whose prerequisites nobody confirmed was written up as checked and failed, or as checked and passed, with no way to say unchecked. It now uses the three the output contract already named. Three more of the same shape: the gate dimensions in `decision-rules.md` listed version, ports, source type and capacity, and not the recovery model or log chain that `DMS-MODE` had been refusing on since v3.14.0, so the rule told a reader to gate the fallback and not what to gate it against; the advisor-to-consumer crosswalk bridged neither field either, so a handoff dropped exactly the two facts `P03-001` and `P24-001` turn on; and the plan validator read whether a question fed a row and never what the answer returned, which let `UNKNOWN` sit under `confirmed`, let an obligation be dismissed as `not_applicable` with no condition named, and let the owed rows be switched off by carrying the path in `selectedMethodPath` and omitting the alias the code happened to read. Six mutations hold those. |
| v3.14.0 | 2026-09-12 | **Three defects found by attacking behaviour instead of surfaces, and every surface was green.** `DMS-MODE` has named `recovery_model` and `log_chain_status` as its inputs since those became typed fields in v3.11.0, and the engine read neither: online DMS came back `available` on a source in SIMPLE recovery with a broken log chain, which is the one state the rule exists to refuse. The same was true of `AVS-LICENSING` and `COPILOT-AGENT`, which name `target_region` and `preview_acceptable` and read nothing. **The check I wrote for exactly this compared the rule index against the schema, not against the engine** — declaring a field and reading it are different things, and it only checked the first. It compares both now, and a scenario carries the refusal so removing the gate fails on a documented case rather than quietly. The plan validator checked the rows a plan carried and never the rows it owed: a `P10` plan cut from nineteen prerequisites to one, with the counts recalculated to agree, came back valid and `ready`. The catalog knows what a path owes and is asked now. And the exemplar plan could not validate against its own output schema in 181 places, because `acceptedEvidence` ids were resolved against `sourceRegister`, which types citations and cannot hold them. There is one evidence model now, `evidenceRegister`, and the exemplar is checked against the schema **and** the invariants in the same run: two checks that had never met. |
| v3.13.1 | 2026-09-12 | **Five findings, and four of them were one file nobody was comparing.** The fork's advisor `SKILL.md` was maintained by hand beside this repository's, not derived from it. That arrangement had already cost a review round, and the fix then was partial: the port carried the capability line and the JSON blocks, and left the prose, which is where the instructions are. So the fork still told the agent that its policy was already in context and that the skill read no file at run time, months after this repository said the opposite and granted read tools. **The whole document is derived now**, with two sections declared fork-owned by heading; anything else that differs is a failure. And the parity checker, which existed, was only ever run by the port itself, so forgetting to port left no trace at all: it runs on every suite now, and fixing something here without porting it fails the build naming the section. The other three: the copyable `advisor_handoff` example in the prerequisite contract had stopped validating against either accepted shape when the mirror gained three required provenance fields, so a user following the canonical page was refused before their path was resolved; a shortlist handoff was required to echo a `controlPlane` that a shortlist cannot carry, leaving the planner to emit invalid JSON or fabricate `standalone`, which is the defaulting defect v3.11.3 removed; and the always-loaded worked exemplar labelled revisitable preferences as technical refusals, used a method gate to decide target eligibility, and claimed `medium` confidence and a General Purpose tier over unconfirmed prerequisites. It is derived from the rules now and reads `low` with the method gate rendered. Writing that found a rule ordering defect: SQL Agent silently upgraded Azure SQL Database out of the refusal linked servers had just given it. 72 gates to 73. |
| v3.13.0 | 2026-09-12 | **Two batches: the canonical fields that were never enumerated, and a verdict nobody had pronounced.** Seven profile fields still accepted any string although the contract tabulates their IDs and the crosswalk maps their exact values, so a plausible normalization variant passed both schemas and fell into the crosswalk's `otherwise`, where a known answer became `UNKNOWN`. Six are enumerated now; `network_ports` is typed as the legacy free text it always was rather than as a canonical answer it never had. **The check that should have caught them read the section headings**, so a heading naming two fields was skipped entirely, and that is exactly where the host, edition, encryption, permission and authentication vocabularies live. It reads the rows now, in both shapes the contract uses, and it found two more things on its first run: a paraphrased `source_os` I had written by hand in the worked example, and a selector the contract tabulates with nowhere in the profile to record what it established, which is now `tier_drivers_state`. **And Phase A: five families start at `unsupported`** and the rules promote what applies, so a family no rule reached kept an initial value that reads as a technical refusal, carrying no rule id and no reason while the trace schema requires both. Families the profile's own preferences explain are `excluded_by_preference` with that reason stated, 65 assertions across 17 scenarios move with them, and a gate refuses any `unsupported` without a recorded reason. Writing it found a rule that had been refusing Azure SQL Database for SQL CLR, Service Broker and cross-database queries without ever saying so. 71 gates to 72. |
| v3.12.2 | 2026-09-12 | **Three review threads that had never been answered, and the reason the last one existed was mine.** Regenerating the worked example in v3.11.5 rebuilt its profile, its candidates and its gate trace, and left `unknowns`, `evidenceRequired` and `blockers` as they were written by hand. So the object contradicted itself: it warned about an unverified Blob upload path the profile had just confirmed, and the hard-gate unknown the rules actually derive, the thirty-day Log Replay Service window, was in neither list although invariant 5 requires both. `blockers` carried a method gate result, so a refused method spoke for target eligibility it was never asked about. All three are derived now. The interview row that merged bandwidth, MI Link ports and Blob reachability is split into the three questions the contract separated them into, for the reason it separated them: one answer covering three gates is how a gate reports `passed` on a fact nobody gave. **And the check that let all of this through compared the card with the JSON and never the JSON with the rules** — the seventh time this round that a check was narrower than its name, and the second on something written the day before. It derives every list the rules produce and compares. Writing the check found one more: the profile said `cross-database queries` where the interview says `cross-DB queries`, a paraphrase I introduced, and the rules match the token — Managed Instance had silently lost its remediation status and Azure SQL Database had stopped being blocked. Free-text profile values are quoted from the interview now, and a check says so; it found a second paraphrase, `TDE-encrypted databases` for `TDE-encrypted DBs`, on its first run. |
| v3.12.1 | 2026-09-12 | **Closing v3.12.0 at the end that reads it.** Making `recommendation.target` an enumeration stopped the Advisor emitting a sentence where a target name belongs, and left the consumer taking any string, so the phrase the producer had just stopped sending would still have been accepted by the skill that receives it. Both ends share one vocabulary for the eight families now, and the regression mirror's `primary_target` with them. Two more asymmetries fell out of looking: the consumer required three fields on a recommendation where the Advisor requires five, so it accepted a handoff without the availability and cutover classes the plan reads; and neither input shape branched on `recommendationStatus`, so a handoff could declare it had refused to choose while naming the target it chose. Both shapes branch now, exactly as the producer does. **The check that missed all three compared the shared vocabularies and never the fields that use them** — a gate whose scope was narrower than its name, for the sixth time this round and the first time on something I had written the day before. It compares the required lists and the resolved typing of every recommendation field now, and the mirror's target with them. |
...
| v3.11.2 | 2026-09-12 | **Four findings that were all the same finding: the output schema types an object nobody had ever built.** A plan could invent a prerequisite id, rewrite the title of the knowledge base row it cites, reclassify a required obligation as recommended, flip `blocking` to false on a real blocker, confirm a row with an evidence id naming no record, cite the same record twice, contradict its own summary counts, and declare itself `ready` over two known-missing blockers. Every one of those was forbidden in prose by the output contract and enforced by nothing. The schema now holds the blocking counts against the verdict, forbids a refusal from carrying a source register, unknowns or assumptions — it already forbade seven plan fields and left four behind — and refuses the same evidence id twice on a row. The rest is not expressible in a schema, so `tools/validate-plan.mjs` derives the counts and the status from the rows, checks every id against the knowledge base, compares each carried row against its source, and resolves every evidence id against the plan's own register. Three invariants were added to say all of that in the contract, 19 to 22. **The reason all four survived is that no plan object existed anywhere in the repository**, so nothing exercised the rules: there is one now, generated from the knowledge base, with thirteen mutations of it that the validator must reject for the stated reason rather than merely reject. And the fork link checker read `.md` and `.json` only, so a reference to a `.mjs` file resolved to nothing and passed; it reads both, and `tools/`, now. |
| v3.11.1 | 2026-09-12 | **One finding, taken on its own because of what it could do.** `knownFacts` is how a caller hands the planner facts it should not ask about again, and it accepted one open union: the readiness enum, any non-empty string, any number, any integer, a boolean and any object. So `tde_status: true`, `database_count: -5`, `fabric_dacpac_size_mb: -1`, `mi_link_ports_status: "BANANA"` and `arc_restore_entrypoint: {}` all validated. A value that validates is a typed fact, and a typed fact confirms a prerequisite, so any of those could clear a blocker on the way to a go decision on a migration. The vocabulary each question accepts was sitting in `questions.json` the whole time and nothing connected the two files. All 77 fields are typed individually now, derived from that file rather than written beside it: an enumerated question types as its own values, a count as an integer of at least one, a size as a number of at least zero, everything else as a string with something in it, and an unrecognised field is refused outright. **Absence and `UNKNOWN` are now different statements**: omitting a key means the question was never asked, `UNKNOWN` means it was asked and not answered, and a check refuses any enumerated field that cannot express the second. A gate derives the whole shape again from `questions.json` and compares, so the two cannot drift. 3345 prerequisite checks to 3552. |
...
| v3.9.0 | 2026-09-11 | **Twenty-six findings from one review round, and the ones that mattered were rules nothing could satisfy or facts nothing could carry.** Two indexed rules read `previewAcceptable` and one read a recovery model the closed profile rejected as an additional property, so a decision those rules made could not be reproduced from a valid input. `preview_acceptable` and `ancillary_services` are now typed fields carried end to end, from the interview question through the contract, the schema, the engine and the profile echo: the existing gates refused each half until all five agreed, which is the chain working. The readiness derivation had no branch for a required non-blocking prerequisite in `reported` state, so a plan whose only outstanding row was a stated fact matched no status at all. **The shipped card and the object it ships with disagreed three ways**: Fabric was excluded as a preview fit although only the Migration Assistant is preview, Arc in-place was eligible in the prose and excluded by preference in the object, and no rendered line carried the rule ID the contract requires. The Markdown is rendered from the object now and a gate compares them. Also: the exemplars emitted `Azure DMS (online)` while the catalog declared `DMS online`, so the handoff resolved on the target alone and could land on the offline plan; four paths advertised a question no prerequisite of theirs consumed; B3 offered three routes the handoff contract refuses without saying so; the card vocabulary omitted `minimal` and `full load time`, so an online DMS recommendation had no token to render with; `prerequisitePaths` accepted `P00` and `P99`; the plan headline could print zero blockers while being held by reported ones; and the skill claimed a startup check its own bundle cannot pass, since three of its seven files never declared both version lines. 59 gates to 66. |
| v3.8.0 | 2026-09-11 | **A reviewer spent a round reporting defects that had been fixed the day before, in a file the port script never read.** The advisor `SKILL.md` in the fork is a transformed copy, so it is patched rather than overwritten, and the patching only touched version stamps and link paths. Corrected rule IDs, the MI Link cutover value, the `blockers` rename and the read-tool capability all stayed upstream. The port now carries what is contract rather than prose: the capability line and every JSON block, rewritten for the fork's layout. **The fork is no longer trusted to be complete; it is proved.** `tools/check-fork-parity.mjs` checks presence, byte-identical content for everything vendored, the JSON a model copies, the rule IDs resolving against the rules the fork itself ships, the retired field names, and every reference resolving in the fork's own layout. It runs at the end of every port, so porting and proving the port are one step. Run against the fork before this release it reported fourteen problems, including all five the reviewer had raised. **Two defects had never been fixed anywhere**, because the JSON-example gate read a hand-written list of one file and one marker: the canonical exemplar carried an empty `normalizedProfile` and two of the six methods the coverage data marks selectable. Blocks are discovered and classified by shape now, and one matching no known shape fails the gate instead of being skipped, which immediately found a third: the prerequisite input contract illustrated an `advisor_handoff` carrying an empty handoff object. A card that recommends must also echo the six fields the input schema requires, since Phase A cannot reach a target without them. 58 gates to 59. |
| v3.7.1 | 2026-09-10 | **The commit that added the code-span path check verified it upstream and not on the fork, where the defect actually was.** The two stale paths a reviewer had flagged are repository-root paths, correct upstream and wrong in the fork once the folders moved, and the port script rewrote only relative ones. So the release that names "verify the set, not the file you changed" as its own lesson shipped with that lesson half applied. The port script rewrites repository-root paths in code spans now, and its verification reads code spans as well as Markdown links: 26 upstream and 25 in the fork, all resolving. Caught by running the independent check against both repositories instead of the one that had just been edited. |
| v3.7.0 | 2026-09-10 | **Nine findings, and six of them were regressions from the two releases shipped the same day.** That number is the release. Every one had the same shape, so this time the mechanism was fixed rather than the instances. A gate written from a fix inherits the blind spot of that fix: the link check shipped in the morning read Markdown links, and the two stale paths that survived the folder rename were written in code spans. The four sweeps added here derive from the **set** of surfaces a token appears on, not from the file that changed. A schema enum value must be named by the page that publishes it, which found a seventh gap nobody had reported: the seven control-plane values had been in the schema since v3.1 and in no contract. A normative field list may not mandate a name no schema declares, which caught `hardBlockers` in two places. Every repository path in a code span must resolve, not only Markdown links. And two fields answering the same question must agree in the fixtures, which caught the selected candidate calling itself `available` while its own gate said otherwise. **The worst finding was a rule that could never be satisfied.** Invariant 19 keyed `confirmed` on `evidenceRequired`, a column the schema requires on all 291 rows, so no typed answer could produce `confirmed` and `ready` was unreachable. The discriminator is derivable instead of invented: 122 prerequisites are fed by a question and settled by an answer, 169 are not and need evidence. A gate proves both populations are non-empty, so the rule has instances on each side. Also: seven rule IDs cited in the shipped exemplars did not exist, teaching a model to fabricate citations that look auditable, and `ruleId` is now an enum generated from the index; invariant 15 demanded the winning method be `available`, which made the ordinary case of a viable method waiting on one unproven field unrepresentable; `unresolved_path` was in the schema and absent from the published vocabulary; and the shipped profile omitted two fields the input schema requires. 56 gates to 58. |
| v3.6.1 | 2026-09-10 | **The mandatory load list named a file that does not exist, and the skill did exactly what it was told: it stopped.** v3.6.0 gave the skills read tools and an instruction to open their policy before evaluating anything, then name the file they could not read and stop. In the same change I wrote the fork's path, `reference/knowledge-base.md`, into the upstream load list, where the prerequisite knowledge base lives under `docs/`. So the first real run refused to produce a plan rather than inventing prerequisites, which is the behaviour working against a path that was wrong. The link is corrected, and the check that would have caught it now runs here: the port script already resolved every relative link against the filesystem, for the fork only. Running a check on one side of a pair and not the other is the defect this repository keeps finding in itself, so it runs on both. 36 links across the skills and the policy documents, each resolved on disk. |

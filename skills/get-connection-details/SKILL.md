...
description: "Connection strings, ports, DNS, firewall and NSG rules, TLS settings and identity prerequisites for Azure SQL Database, Azure SQL Managed Instance, SQL Server on Azure VM, Fabric SQL database and Fabric Warehouse, plus root-cause diagnosis when a connection is refused. Covers the public endpoint on port 3342 versus VNet-local and private endpoints on 1433, Redirect and Proxy connection policies and the port ranges each needs, privatelink DNS zones, Microsoft Entra authentication keywords per driver (SqlClient, JDBC, ODBC, sqlcmd) with their minimum versions, and errors 18456, 40615, 40532, 4060, 40613 and connection timeouts. Trigger when the user cannot connect or gets a timeout, login failure or firewall error against an Azure SQL target, when a connection works from one machine or network but not another, when they ask which port, which connection string, which NSG or firewall rule, or which authentication keyword to use, or say 'can't connect to SQL', 'connection refused', 'login failed', 'connexion impossible' or 'quel port ouvrir'. This is a network, endpoint, identity and driver-configuration problem, not an application code bug: prefer this over a general debugging skill whenever the failing component is an Azure SQL connection."

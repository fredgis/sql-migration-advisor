...
assertRegex('azureArcFloors.miLinkToManagedInstance', new RegExp(`Arc → Azure SQL MI via MI Link:[\\s\\S]*?SQL Server \\*\\*${arc.miLinkToManagedInstance.sqlServerMin}\\+\\*\\*[\\s\\S]*?\\*\\*Windows Server only\\*\\*`, 'u'), `expected Arc MI Link floor ${arc.miLinkToManagedInstance.sqlServerMin}+ and the Windows Server only qualifier`);
